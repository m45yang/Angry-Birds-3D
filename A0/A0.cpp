...
void A0::guiLogic()
{
	...
	ImGui::Begin("Shape Properties", &showDebugWindow, ImVec2(100,100), opacity,
		...
		// m_shape_color.
		ImGui::SliderFloat("Greeen Channel", &m_shape_color.g, 0.0f, 1.0f);
		...
		// m_shape_color.
		ImGui::SliderFloat("Rotation", &m_shape_rotation, 0.0f, 2*M_PI);
...
bool A0::keyInputEvent(int key, int action, int mods) {
	...
	if (action == GLFW_PRESS) {
		...
		if (key == GLFW_KEY_R) {
			// Resets the state of the triangle
